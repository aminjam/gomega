...
var _ = Describe("LogEntry", func() {
	...
	var entry gmeasure.LogEntry

	BeforeEach(func() {
		e = gmeasure.NewExperiment("Test Experiment")
	})

	Describe("Note LogEntry", func() {
		BeforeEach(func() {
		...
		Describe("Generating Stats", func() {
			It("returns an empty stats", func() {
				Ω(entry.Stats()).Should(BeZero())
			})
		})

		Describe("Emitting an unstyled report", func() {
			It("does not include styling", func() {
				Ω(entry.String()).Should(Equal("Test Experiment - Note\nI'm a red note\n"))
			})
		})

		Describe("Emitting a styled report", func() {
			It("does include styling", func() {
				Ω(entry.ColorableString()).Should(Equal("{{red}}Test Experiment - Note\nI'm a red note\n{{/}}"))
			})
		})
	})

	Describe("Value LogEntry", func() {
		...
		Describe("Generating Stats", func() {
			It("generates a correctly configured Stats with correct values", func() {
				...
				Ω(stats.Name).Should(Equal("flange widths"))
		...
		Describe("Emitting an unstyled report", func() {
			It("does not include styling", func() {
				...
				Ω(entry.String()).Should(Equal(expected))
			})
		})
		...
		Describe("Emitting a styled report", func() {
			It("does include styling", func() {
				...
				Ω(entry.ColorableString()).Should(Equal(expected))
			})
		})
	...
	Describe("Duration LogEntry", func() {
		...
		Describe("Generating Stats", func() {
			It("generates a correctly configured Stats with correct values", func() {
				stats := entry.Stats()
				Ω(stats.ExperimentName).Should(Equal("Test Experiment"))
				Ω(stats.Name).Should(Equal("runtime"))
				Ω(stats.Style).Should(Equal("{{blue}}"))
