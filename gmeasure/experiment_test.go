...
var _ = Describe("Experiment", func() {
	...
	Describe("Recording Notes", func() {
		It("createas a note LogEntry", func() {
			e.RecordNote("I'm a note", gmeasure.Style("{{blue}}"))
			entry := e.LogEntries[0]
			Ω(entry.Type).Should(Equal(gmeasure.LogEntryTypeNote))
			Ω(entry.ExperimentName).Should(Equal("Test Experiment"))
			Ω(entry.Note).Should(Equal("I'm a note"))
			Ω(entry.Style).Should(Equal("{{blue}}"))
		})
	})

	Describe("Recording Durations", func() {
		commonEntryAssertions := func() gmeasure.LogEntry {
			entry := e.Get("runtime")
			Ω(entry.Type).Should(Equal(gmeasure.LogEntryTypeDuration))
			Ω(entry.ExperimentName).Should(Equal("Test Experiment"))
			Ω(entry.Name).Should(Equal("runtime"))
			Ω(entry.Units).Should(Equal("duration"))
			Ω(entry.Style).Should(Equal("{{red}}"))
			Ω(entry.PrecisionBundle.Duration).Should(Equal(time.Millisecond))
			return entry
		}
		...
		Describe("RecordDuration", func() {
			It("generates a log-entry and records the passed-in duration along with any relevant decorations", func() {
				e.RecordDuration("runtime", time.Minute, gmeasure.Annotation("second"))
				entry := commonEntryAssertions()
				Ω(entry.Durations).Should(Equal([]time.Duration{time.Second, time.Minute}))
				Ω(entry.Annotations).Should(Equal([]string{"first", "second"}))
			})
		})
		...
		Describe("MeasureDuration", func() {
			It("measure the duration of the passed-in function", func() {
				...
				entry := commonEntryAssertions()
				Ω(entry.Durations[0]).Should(Equal(time.Second))
				Ω(entry.Durations[1]).Should(BeNumerically("~", 200*time.Millisecond, 20*time.Millisecond))
				Ω(entry.Annotations).Should(Equal([]string{"first", "second"}))
			})
		})
		...
		Describe("SampleDuration", func() {
			It("samples the passed-in function according to SampleConfig and records the measured durations", func() {
				...
				entry := commonEntryAssertions()
				Ω(entry.Durations[0]).Should(Equal(time.Second))
				Ω(entry.Durations[1]).Should(BeNumerically("~", 100*time.Millisecond, 20*time.Millisecond))
				Ω(entry.Durations[2]).Should(BeNumerically("~", 100*time.Millisecond, 20*time.Millisecond))
				Ω(entry.Durations[3]).Should(BeNumerically("~", 100*time.Millisecond, 20*time.Millisecond))
				Ω(entry.Annotations).Should(Equal([]string{"first", "sampled", "sampled", "sampled"}))
			})
		})
		...
		Describe("SampleAnnotatedDuration", func() {
			It("samples the passed-in function according to SampleConfig and records the measured durations and returned annotations", func() {
				...
				Ω(entry.Durations[2]).Should(BeNumerically("~", 100*time.Millisecond, 20*time.Millisecond))
				Ω(entry.Durations[3]).Should(BeNumerically("~", 100*time.Millisecond, 20*time.Millisecond))
				Ω(entry.Annotations).Should(Equal([]string{"first", "sampled-1", "sampled-2", "sampled-3"}))
			})
		})
	})
	...
	Describe("Stopwatch Support", func() {
		It("can generate a new stopwatch tied to the experiment", func() {
			...
			entry := e.Get("runtime")
			Ω(entry.Durations[0]).Should(BeNumerically("~", 50*time.Millisecond, 20*time.Millisecond))
			Ω(entry.Durations[1]).Should(BeNumerically("~", 100*time.Millisecond, 20*time.Millisecond))
			Ω(entry.Durations[2]).Should(BeNumerically("~", 150*time.Millisecond, 20*time.Millisecond))
			Ω(entry.Annotations).Should(Equal([]string{"first", "second", "third"}))
		})
	})

	Describe("Recording Values", func() {
		commonEntryAssertions := func() gmeasure.LogEntry {
			entry := e.Get("sprockets")
			Ω(entry.Type).Should(Equal(gmeasure.LogEntryTypeValue))
			Ω(entry.ExperimentName).Should(Equal("Test Experiment"))
			Ω(entry.Name).Should(Equal("sprockets"))
			Ω(entry.Units).Should(Equal("widgets"))
			Ω(entry.Style).Should(Equal("{{yellow}}"))
			Ω(entry.PrecisionBundle.ValueFormat).Should(Equal("%.0f"))
			return entry
		}
		...
		Describe("RecordValue", func() {
			It("generates a log-entry and records the passed-in value along with any relevant decorations", func() {
				e.RecordValue("sprockets", 17.4, gmeasure.Annotation("second"))
				entry := commonEntryAssertions()
				Ω(entry.Values).Should(Equal([]float64{3.2, 17.4}))
				Ω(entry.Annotations).Should(Equal([]string{"first", "second"}))
			})
		})
		...
		Describe("SampleValue", func() {
			It("samples the passed-in function according to SampleConfig and records the resulting values", func() {
				...
				entry := commonEntryAssertions()
				Ω(entry.Values).Should(Equal([]float64{3.2, 17.4, 18.4, 19.4}))
				Ω(entry.Annotations).Should(Equal([]string{"first", "sampled", "sampled", "sampled"}))
			})
		})
	...
	Describe("recording multiple entries", func() {
		It("always appends to the correct entry (by name)", func() {
			...
			Ω(e.LogEntries).Should(HaveLen(4))
	...
	Describe("Getting LogEntries", func() {
		Context("when the LogEntry does not exist", func() {
			It("returns the zero LogEntry", func() {
	...
	Describe("Getting Stats", func() {
		It("returns the LogEntry's Stats", func() {
